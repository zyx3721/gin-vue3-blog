...
邮件订阅功能中的退订链接、文章链接等需要使用博客前台地址。**如不使用邮件订阅功能,可以不配置此项,不影响其他功能。**

**开发环境** (`config/config-dev.yml`)：
```yaml
app:
  port: 8080
...
**生产环境** (`config/config-prod.yml`)：
```yaml
app:
  port: 8080
  blog_url: "https://your-domain.com"  # 必须修改为实际域名
```
...
> **注意**：
> - 仅使用邮件订阅功能时需要配置此项
> - 生产环境部署时,必须在 `config-prod.yml` 中配置正确的 `blog_url`
> - 如不配置,邮件中的退订链接和文章链接将无法正常工作
> - 其他功能(文章、评论、聊天室、用户管理等)不受影响
